	...
	tinymce.create('tinymce.plugins.IcePlugin', {
		...
		init: function(ed, url) {
			...
			ed.onPostRender.add(function(mgr) {
				...
				self.boldSelector = '.' + self.boldClass;	
				self.italicSelector = '.' + self.italicClass;
				self.underlineSelector = '.' + self.underlineClass;			
				...
				ed.serializer.addRules(self.boldTag + '[id|class|title|'+self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
				ed.serializer.addRules(self.italicTag + '[id|class|title|'+self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
				ed.serializer.addRules(self.underlineTag + '[id|class|title|'+self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
				...
				tinymce.DOM.win.setTimeout(function() {
					changeEditor = new ice.InlineChangeEditor({
						...
						changeTypes: {
							...
							boldType: {tag: self.boldTag, alias: self.boldClass},
							italicType: {tag: self.italicTag, alias: self.italicClass},
							underlineType: {tag: self.underlineTag, alias: self.underlineClass}
						}
					}).startTracking();
			...
			/**
			 * Set the bold tracking code.
			 */
			ed.addCommand('ice_bold', function(insertBold) {
				insertBold = insertBold || {};
				changeEditor.insertBold('\u00A0' , insertBold.range);
			});
			/**
			 * Set the italic tracking code.
			 */
			ed.addCommand('ice_italic', function(insertItalic) {
				insertItalic = insertItalic || {};
				changeEditor.insertItalic('\u00A0' , insertItalic.range);
			});
			/**
			 * Set the underline tracking code.
			 */
			ed.addCommand('ice_underline', function(insertUnderline) {
				insertUnderline = insertUnderline || {};
				changeEditor.insertUnderline('\u00A0' , insertUnderline.range);
			});
